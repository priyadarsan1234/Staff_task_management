...
class _TeacherFeedbackState extends State<TeacherFeedbackpage> {
  ...
  String teacherId = ''; // Variable to store teacher ID

  @override
  void initState() {
    super.initState();
    _loadTeacherId(); // Load teacher ID before fetching data
  }
  ...
  Future<void> _loadTeacherId() async {
    ...
    futureData = fetchData(); // Fetch data after loading teacher ID
  }
  ...
  Map<String, List<Map<String, dynamic>>> filterData(
    ...
    if (course == 'All') {
      ...
      data.forEach((week, items) {
        final filteredItems = items.where((item) => item['teacher_id'] == teacherId).toList();
    ...
    final filteredData = <String, List<Map<String, dynamic>>>{};
    data.forEach((week, items) {
      final filteredItems = items.where((item) => item['cource'] == course && item['teacher_id'] == teacherId).toList();
      if (filteredItems.isNotEmpty) {
        filteredData[week] = filteredItems;
      }
    });
